    ...
    dto = WorkflowDefinitionDTO.model_validate({
        ...
        "tasks": {
            ...
            "task_b": {
                ...
                "input_bindings": {
                    "val": {"type": "task_output", "task": "task_a"}
                },
                "max_attempts": 1,
            },
        },
...
def test_workflow_output_source_must_exist():
    dto = WorkflowDefinitionDTO.model_validate({
        ...
        "output_bindings": {
            "out": {"type": "task_output", "task": "unknown_task"}
        },
    })
    candidate = normalize_workflow_dto(dto)
    outcome = SemanticValidator().validate(candidate)
...
def test_cycle_rejected():
    dto = WorkflowDefinitionDTO.model_validate({
        ...
        "tasks": {
            "task_a": {"activity_type": "test.act", "dependencies": ["task_b"], "max_attempts": 1},
            "task_b": {"activity_type": "test.act", "dependencies": ["task_a"], "max_attempts": 1},
        },
    })
    candidate = normalize_workflow_dto(dto)
    outcome = SemanticValidator().validate(candidate)
...
def test_multiple_roots_and_leaves_accepted():
    dto = WorkflowDefinitionDTO.model_validate({
        ...
        "tasks": {
            ...
            "middle": {"activity_type": "test.act", "dependencies": ["root_1", "root_2"], "max_attempts": 1},
            "leaf_1": {"activity_type": "test.act", "dependencies": ["middle"], "max_attempts": 1},
            "leaf_2": {"activity_type": "test.act", "dependencies": ["middle"], "max_attempts": 1},
        },
    })
    candidate = normalize_workflow_dto(dto)
    outcome = SemanticValidator().validate(candidate)
